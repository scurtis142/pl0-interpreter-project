...
public class Parser {
    ...
    /**
     * Set of tokens that may start a Do Branch/
     */
    ...
    private final ParseMethod<StatementNode.DoNode.DoBranch> doBranch =
            new ParseMethod<>((Location loc) -> {
                return new StatementNode.DoNode.DoBranch(loc,
    ...
    /**
     * Methods for parsing list of expressions return an List<ExpNode>,
     * even if there is an error, in which case they'll return
     * an ExpNode.ErrorNode.
     */
    private final ParseMethod<List<ExpNode>> listexp =
        new ParseMethod<List<ExpNode>>((Location loc) -> {
            List<ExpNode> listexp = new LinkedList<>();
            listexp.add(new ExpNode.ErrorNode(loc));
            return listexp;
        });
    ...
    private List<ExpNode> parseLValueList(TokenSet recoverSet) {
        return listexp.parse("LValue List", LVALUE_START_SET, recoverSet,
                () -> {
                    /* The current token is in LVALUE_START_SET.
